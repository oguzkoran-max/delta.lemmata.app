...
| EPI-07 | High | Genre, audience, publisher veya chronology confound'u stil değişimi gibi yorumlanır. | Confound inventory; balanced/stratified comparison mümkünse; negative control; warning language | Diachronic benchmark ve worked-example report | P004, P010, P013 |
...
|---|---|---|---|
| SEC-05 | `UI candidate locally verified; human acceptance pending` | `corpus-metadata-csv-v1` 58 sütunla P003 limitinin altında; untrusted import, generated metadata export, composition CSV ve completeness CSV aynı P003 formül, HTML, newline ve path reddinden geçecek biçimde uygulanmış; review CSV'leri yalnız hash-bound payload-free projection'dan üretiliyor; JSON içi Unicode-escaped değerler decode sonrasında BOM, bidi, control ve NFC kontrolleri dahil aynı politikayla yeniden taranıyor; duplicate key, non-standard constant ve aşırı nesting fail-closed; Guided import diagnostics raw cell veya TXT içeriği göstermiyor; accepted ZIP için yalnız safe member label/hash/count summaries P004 state'ine taşınıyor, archive/member bytes ve storage name taşınmıyor; individual-TXT + ZIP browser audit no external host, console error veya payload echo bulmadı | Exact-commit/CI ve human negative-flow walkthrough; sonuç ve FAIR paket export'u P009/P012 |
| RP-01 | `model/CSV/UI candidate locally verified; human acceptance pending` | Her analyzed asset tam `asset-rights-v2` dependency array'i taşır; rights kaynağı acquisition kaynağından farklıysa ayrı source kaydı korunur; guided questionnaire state'i dört ayrı eyleme deterministik map eder; Rights Action Matrix aynı canonical inventory'den üretilir; unknown ve permission-required fail-closed kalır | Oğuz terminology/negative-flow walkthrough, complete visual review ve Collodi item-level dossier P004/P013 |
| RP-02 | `model/CSV/UI candidate locally verified; export package pending` | Upload, analysis, export ve public redistribution izinleri ayrı korunur; public redistribution için verified-open status, export izni, license, jurisdiction ve URL evidence birlikte zorunludur; statement-only claim kapıyı açamaz; UI matrix action-specific permission'ı text ile gösterir ve default documentation download raw text içermez | Raw export kapısı ve public package negatif fixture'ları P012/P013 |
| EPI-01 | `model/CSV/review interaction locally verified; human acceptance pending` | Exact file label + SHA-256 P003 catalog bağı individual TXT ve validated ZIP member için aynı deterministic P004 unit'e projekte edilir; stable work/edition/source kimliği, row-aware validation ve canonical round trip korunur; guided editor canonical inventory üretir ve payload-free input state ile Review dönüşünde değer kaybetmez; hash-bound completeness matrix identity, chronology, edition, source, classification, rights ve normalization alanlarını score üretmeden görünür yapar; non-complete field path exact guided work/section'a veya source CSV `work_id` + field düzeltmesine yönlenir; final mapping/rights acknowledgement inventory hash'ine bağlıdır ve rebuild ile invalidated olur | Paratext ve ileri corpus-health kontrolleri P007; Oğuz field-correction, ZIP ve confirmation walkthrough'u P004 |
| EPI-07 | `warning/blocker and selectable visual projection locally verified; human acceptance pending` | Unknown genre, audience, adaptation, collection ve source confound olarak görünür kalır; her composition boyutu tüm work kayıtlarını paydada tutar; görsel, semantic table ve CSV aynı projection key'lerini paylaşır; selectable chronology yalnız documented date/order gösterir ve distance/trajectory/quality kodlamaz; Style Over Time mixed author set ve mixed language durumlarını blocker yapar; readiness threshold ve completeness state bilimsel yeterlilik diye sunulmaz | Diachronic benchmark, worked-example report ve Oğuz visual terminology acceptance P004/P010/P013 |
